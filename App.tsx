...
import { Upload, Users, CheckCircle2, FileText, Vote, X, AlertCircle, RotateCcw, ArrowRight, Zap, Search, Keyboard, CornerDownLeft } from 'lucide-react';
// @ts-ignore
import Pinyin from 'tiny-pinyin';
import { Candidate, AppState } from './types';
import { parseCandidateList, groupCandidatesByDept } from './utils/parser';
  ...
  if (appState === 'setup') {
    return (
      <div className="min-h-screen flex flex-col bg-[#f2f2f2] relative overflow-hidden">
        ...
        <div className="absolute bottom-[-10%] right-[-10%] w-[500px] h-[500px] bg-blue-300 rounded-full mix-blend-multiply filter blur-[128px] opacity-40 animate-blob animation-delay-2000"></div>

        <div className="flex-1 flex items-center justify-center w-full z-10 p-6">
          <div className="bg-white/80 backdrop-blur-xl p-8 md:p-12 rounded-[2rem] shadow-2xl w-full max-w-4xl border border-white/50 relative grid grid-cols-1 md:grid-cols-2 gap-12">
            
            <div className="flex flex-col justify-center">
              <div className="inline-flex items-center gap-2 bg-black text-white px-4 py-1.5 rounded-full text-xs font-bold w-fit mb-6">
              ...
              <h1 className="text-5xl md:text-6xl font-black text-slate-900 leading-tight tracking-tight mb-6">
                学生会<br/>
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-violet-600 to-indigo-600">部长竞选</span>
                <br/>计票系统
              </h1>
              ...
              <div className="flex gap-4">
                ...
                >
                  <div className="bg-slate-100 p-2 rounded-lg group-hover:bg-violet-100 transition-colors">
            ...
            <div className="flex flex-col h-full bg-slate-50 rounded-3xl p-2 border border-slate-100 shadow-inner">
  ...
  const Header = () => (
    <header className="sticky top-0 z-40 bg-slate-900/90 backdrop-blur-xl border-b border-slate-800">
      <div className="max-w-[1600px] mx-auto px-6 h-20 flex items-center justify-between">
        ...
        {appState === 'results' && (
           <button onClick={() => setAppState('voting')} className="text-sm font-semibold text-slate-400 hover:text-white transition-colors">
             ← 返回计票
           </button>
        )}
      </div>
    ...
    return (
      <div className="min-h-screen bg-[#f8f9fc] flex flex-col font-sans relative">
        ...
        <main className="flex-1 p-6 md:p-8 overflow-y-auto flex flex-col">
          <div className="max-w-[1600px] mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 w-full mb-auto">
        ...
        <div className="fixed bottom-32 left-0 right-0 z-30 px-4 flex justify-center">
            <div className="w-full max-w-2xl relative">
                ...
                {quickVoteInput && matchedCandidates.length > 0 && (
                    <div className="absolute bottom-full left-0 right-0 mb-3 bg-white/95 backdrop-blur-md border border-slate-200 rounded-2xl shadow-2xl overflow-hidden animate-in slide-in-from-bottom-2 fade-in duration-200">
                        <div className="flex items-center justify-between px-4 py-2 bg-slate-50 border-b border-slate-100">
                           <div className="text-xs font-bold text-slate-400 uppercase tracking-wider">
                              匹配到 {matchedCandidates.length} 位候选人
                           </div>
                           <div className="flex items-center gap-1 text-[10px] font-medium text-slate-400 bg-white px-2 py-1 rounded border">
                        ...
                        <div ref={suggestionsListRef} className="max-h-[240px] overflow-y-auto custom-scrollbar">
                ...
                {lastVotedCandidate && (
                   <div className="absolute -top-20 left-1/2 -translate-x-1/2 z-50">
                      <div className="bg-slate-900 text-white px-6 py-3 rounded-2xl shadow-xl flex items-center gap-3 animate-bounce-short whitespace-nowrap">
                ...
                >
                    <div className="pl-4 pr-3 text-slate-400">
                        {quickVoteInput ? <Keyboard size={24} className="text-violet-500" /> : <Search size={24} />}
                    </div>
                    <input
        ...
        {showFinishModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="absolute inset-0 bg-slate-900/30 backdrop-blur-sm" onClick={() => setShowFinishModal(false)} />
            <div className="bg-white rounded-3xl p-8 w-full max-w-sm relative z-10 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
              <div className="w-12 h-12 bg-lime-100 rounded-full flex items-center justify-center mb-4 text-lime-600">
        ...
        {showResetModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="absolute inset-0 bg-slate-900/30 backdrop-blur-sm" onClick={() => setShowResetModal(false)} />
            <div className="bg-white rounded-3xl p-8 w-full max-w-sm relative z-10 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
